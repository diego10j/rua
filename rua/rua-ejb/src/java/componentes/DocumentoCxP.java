...
public class DocumentoCxP extends Dialogo {
    ...
    public DocumentoCxP() {
        ...
        parametros = utilitario.getVariables("ide_usua", "ide_empr", "ide_sucu",
                ...
                "p_sri_adqui_pagos_12%_dertri_501",
                "p_sri_adqui_local_12%_dertri_502",
    ...
    public void seleccionarArchivoXML(FileUploadEvent event) {
        ...
        try {
            ...
            for (String strDetalleActual : strDetalles) {
                ...
                tab_det_documento.setValor("cantidad_cpdfa", utilitario.getFormatoNumero(utilitario.getValorEtiqueta(strDetalleActual, "cantidad"), 3));
    ...
    private void calcularTotalDetalleDocumento() {
        ...
        tab_det_documento.setValor("valor_cpdfa", utilitario.getFormatoNumero(utilitario.getFormatoNumero(total, 4)));
...
////        }
        ...
        List lis_numeros_fact = utilitario.getConexion().consultar("select numero_cpcfa,autorizacio_cpcfa from cxp_cabece_factur where numero_cpcfa='" + tab_cab_documento.getValor("numero_cpcfa") + "' and ide_geper=" + tab_cab_documento.getValor("ide_geper") + " and autorizacio_cpcfa='" + tab_cab_documento.getValor("autorizacio_cpcfa") + "'  and ide_cpefa=0");
...
//        }
    ...
    public void obteneAlterno104() {
        try {
            ...
            String ide_cntco = ser_proveedor.getParametroProveedor("ide_cntco", tab_cab_documento.getValor("ide_geper"));
            boolean boo_tipo_rise = false;
            if (ide_cntco != null) {
                if (ide_cntco.equals(parametros.get("p_con_tipo_contribuyente_rise"))) {
                    boo_tipo_rise = true;
                }
            }
            ...
            if (boo_tipo_rise == false) {
                for (int i = 0; i < tab_det_documento.getTotalFilas(); i++) {
                    if (tab_det_documento.getValor(i, "ide_inarti") != null && !tab_det_documento.getValor(i, "ide_inarti").isEmpty()) {
                        tipo_articulo = ser_producto.getTipoProducto(tab_det_documento.getValor(i, "ide_inarti"));
                        //aplica_iva = obtener_aplica_iva_producto(tab_det_documento.getValor(i, "ide_inarti"));
                        if (tipo_articulo != null && !tipo_articulo.equals(p_activos_fijos) && tab_det_documento.getValor(i, "iva_inarti_cpdfa").equals("1") && tab_det_documento.getValor(i, "credi_tribu_cpdfa").equals("true")) {
                            tab_det_documento.setValor(i, "alter_tribu_cpdfa", parametros.get("p_sri_adqui_pagos_12%_dertri_501"));
                            continue;
                        }
                        if (tab_det_documento.getValor(i, "iva_inarti_cpdfa").equals("1") && tab_det_documento.getValor(i, "credi_tribu_cpdfa").equals("true")) {
                            tab_det_documento.setValor(i, "alter_tribu_cpdfa", parametros.get("p_sri_adqui_local_12%_dertri_502"));
                        } else if (tab_det_documento.getValor(i, "iva_inarti_cpdfa").equals("1") && tab_det_documento.getValor(i, "credi_tribu_cpdfa").equals("false")) {
                ...
                for (int i = 0; i < tab_det_documento.getTotalFilas(); i++) {
                    tab_det_documento.setValor(i, "alter_tribu_cpdfa", "508");
                }
            }
        } catch (Exception e) {
        }
